...
SELECT DISTINCT A.A_NAME
 ...
 WHERE A.A_ID IN (SELECT C.A_ID 
                   ...
                   WHERE C.E_ID = E.E_ID 
                   AND NOT EXISTS ( SELECT * FROM EMPLOYEE E1 WHERE E1.E_ID = E.E_ID AND E1.SALARY <80000 ));
                   
                   SELECT C.E_ID,MAX(A.CRUISING_RANGE) AS MAX_CRUSING_RANGE
                   FROM CERTIFIED C,AIRCRAFT A
                   WHERE C.A_ID=A.A_ID AND C.E_ID=(SELECT E_ID FROM CERTIFIED GROUP BY E_ID HAVING COUNT(*)>3);
                   
                   SELECT E_NAME 
                   FROM EMPLOYEE WHERE E_ID IN (SELECT E_ID FROM CERTIFIED)
                   AND SALARY<(SELECT MIN(PRICE)
                   FROM FLIGHTS 
                   WHERE FROM_PLACE='BENGALURU' AND TO_PLACE='FRANKFURT');
                   
                   SELECT A.A_NAME,A.A_ID,AVG(E.SALARY) AS AVERAGE_SALARY
                   FROM EMPLOYEE E,CERTIFIED C , AIRCRAFT A
                   WHERE E.E_ID=C.E_ID AND C.A_ID IN (SELECT A_ID FROM AIRCRAFT WHERE CRUISING_RANGE>=1000) GROUP BY C.A_ID;
                   
                   SELECT A_ID FROM AIRCRAFT
                   WHERE CRUISING_RANGE>(SELECT DISTANCE FROM FLIGHTS WHERE FROM_PLACE='BENGALURU' AND TO_PLACE='NEW DELHI');
                   
                   SELECT DISTINCT E.e_name
FROM Employee E, Certified C, Aircraft A
WHERE E.e_id = C.e_id AND C.a_id = A.a_id AND A.a_name LIKE "Boeing";

SELECT A_ID
 FROM AIRCRAFT 
 WHERE CRUISING_RANGE>(SELECT DISTANCE
 FROM FLIGHTS
 WHERE FROM_PLACE='BENGALURU' AND TO_PLACE='NEW DELHI');
 
 SELECT E_NAME,SALARY
 FROM EMPLOYEE
 WHERE E_ID NOT IN(SELECT E_ID FROM CERTIFIED) 
 AND SALARY>(SELECT AVG(SALARY)
 FROM EMPLOYEE WHERE E_ID IN (SELECT E_ID FROM CERTIFIED));
 
 SELECT F.DEPARTS,F.FROM_PLACE,F.TO_PLACE
 FROM FLIGHTS F
 WHERE F.FL_ID IN ( ( SELECT F0.FL_ID FROM FLIGHTS F0 WHERE F0.FROM_PLACE = 'MANDISION' AND F0.TO_PLACE = 'NEW YORK' 
